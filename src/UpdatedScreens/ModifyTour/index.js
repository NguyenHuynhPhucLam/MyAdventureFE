...
const ModifyTour = (props) => {
  ...
  useEffect(() => {
    if(tour && tour._id) {
        ...
        const departureDateTime = moment(tour.departureDate, moment.ISO_8601).format('DD-MM-YYYY HH:mm');
        setDepartureDate(moment(departureDateTime, 'DD-MM-YYYY HH:mm').toDate());
        const returnDateTime = moment(tour.returnDate, moment.ISO_8601).format('DD-MM-YYYY HH:mm');
        setReturnDate(moment(returnDateTime, 'DD-MM-YYYY HH:mm').toDate());
        setAccomodation(tour.hotel)
        const checkinDateTime = moment(tour.checkin, moment.ISO_8601).format('DD-MM-YYYY HH:mm');
        setCheckinDate(moment(checkinDateTime, 'DD-MM-YYYY HH:mm').toDate());
        const checkoutDateTime = moment(tour.checkout, moment.ISO_8601).format('DD-MM-YYYY HH:mm');
  ...
  const UpdateTour = async (data) => {
    ...
    const handleSubmit = async (e) => {
      if(transportBrand === "" || accomodation === "" || departureDate === undefined || returnDate === undefined || checkinDate === undefined || checkoutDate === undefined || price === "" || details === "") {
